  ...
  listeners = {
    ...
    alb-https-listner = {
      ...
      fixed_response = {
        ...
        message_body = "Request denied - Please use the relevent domain name to access"
        status_code  = "200"
      }
      ...
      rules = {
        rule-01-weighted-forward = {
          ...
          conditions = [{
            host_header = {
              values = ["${var.alb_app1_dns_name}.${data.aws_route53_zone.cv_domain.name}"]
            }
          }]
        }
        rule-02-weighted-forward = {
          ...
          conditions = [{
            host_header = {
              values = ["${var.alb_app2_dns_name}.${data.aws_route53_zone.cv_domain.name}"]
            }
          }]
        }
      }
