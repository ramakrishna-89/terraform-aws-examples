...
resource "aws_route53_record" "cv_subdomain_app1" {
  zone_id = data.aws_route53_zone.cv_domain.zone_id
  name    = "${var.alb_app1_dns_name}.${data.aws_route53_zone.cv_domain.name}"
  type    = "A"
  alias {
    name                   = module.alb.dns_name
    zone_id                = module.alb.zone_id
...
resource "aws_route53_record" "cv_subdomain_app2" {
  zone_id = data.aws_route53_zone.cv_domain.zone_id
  name    = "${var.alb_app2_dns_name}.${data.aws_route53_zone.cv_domain.name}"
  type    = "A"
  alias {
    name                   = module.alb.dns_name
    zone_id                = module.alb.zone_id
