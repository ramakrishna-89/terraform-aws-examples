  ...
  listeners = {
    ...
    alb-https-listner = {
      ...
      fixed_response = {
        ...
        message_body = "Request denied - Please use the relevent context path/domain to access"
        status_code  = "200"
      }
      ...
      rules = {
        ...
        rule-03-weighted-forward = {
          priority = 3
          actions = [{
            type = "weighted-forward"
            target_groups = [
              {
                target_group_key = "tg-03"
                weight           = 1
            }]
            stickiness = {
              enabled  = true
...
resource "aws_lb_target_group_attachment" "tg-03-attachment" {
  for_each         = { for k, v in module.ec2_private_instance_app3 : k => v }
  target_group_arn = module.alb.target_groups["tg-03"].arn
  target_id        = each.value.id
  port             = 8080
}
