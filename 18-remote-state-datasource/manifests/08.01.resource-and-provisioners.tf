...
resource "null_resource" "null_resource" {
  ...
  connection {
    type        = "ssh"
    host        = aws_eip.ec2_eip_public_instance.public_ip
    user        = "ec2-user"
    password    = ""
  ...
  provisioner "local-exec" {
    ...
    command     = "echo VPC created on `date` and VPC ID: ${data.terraform_remote_state.vpc.outputs.vpc_id} >> vpc-creation-stamp.txt"
    #on_failure = continue
  }
